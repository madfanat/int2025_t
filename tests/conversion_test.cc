...
TEST(FromInt, MinimumInt32MatchesDecimalConversion) {
  const int2025_t from_integer =
      from_int(std::numeric_limits<std::int32_t>::min());

  EXPECT_EQ(from_integer, from_string("-2147483648"));
}

TEST(FromInt, NegativeOneHasCanonicalPadding) {
  const int2025_t result = from_int(-1);

  for (std::size_t i = 0; i + 1 < kBytes; ++i) {
    EXPECT_EQ(result.value[i], 255);
  }
  EXPECT_EQ(result.value[kBytes - 1], 1);
}
...
TEST(FromString, ReallyBigOverflow630Digits) {
  int2025_t result1 = from_string(
      "123456789012345678901234567890123456789012345678901234567890123456789012"
      "345678901234567890123456789012345678901234567890123456789012345678901234"
      "567890123456789012345678901234567890123456789012345678901234567890123456"
